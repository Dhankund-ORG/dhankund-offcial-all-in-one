...
The company provides loan services, consulting services, and is currently building a CRM, a DSA platform, and an official site—ALL housed within a single monorepo (one repository) targeting Web, Android, and iOS platforms.
...
- **Frontend/App Framework:** Flutter (Web, Android APK/AAB builds, and iOS builds).
...
1. **Multi-Project Environment:** This single repository contains multiple distinct projects (CRM, DSA platform, and Official Website) across three platforms (Web, Android, iOS).
...
1. **Public Repository Constraint:** The repository is PUBLIC. NEVER hardcode API keys, service account JSON files, Apple certificates, or any sensitive data in the codebase.
2. **Universal GitHub Secrets:** ALL credentials—including Firebase configurations, Cloudflare API tokens, and Google Play Store Console / Apple App Store credentials—MUST be securely stored in and accessed ONLY via GitHub Secrets (e.g., `${{ secrets.SECRET_NAME }}`). 
   - This rule strictly applies to ALL platforms (Web, Android, iOS) and their respective build/deployment workflows. The code and workflows must fetch credentials dynamically from GitHub Secrets.
...
4. **Push Notifications (FCM):** When implementing or updating Firebase Cloud Messaging (FCM) for push notifications across Web, Android, or iOS, you MUST strictly read and follow the LATEST official Firebase documentation. Implementations must align with the most current best practices provided by Firebase.
...
1. **Separate Workflows:** Always create independent, dedicated GitHub workflow files for different tasks (e.g., web deploy vs Android APK/AAB build vs iOS IPA build).
...
4. **Architecture-Aware Build & Deployment:**
   ...
   - **Dynamic Build:** Adapt the workflow dynamically based on the platform (e.g., `flutter build web`, `flutter build apk`, or iOS builds).
   - **Deploy:** Deploy frontend to Cloudflare Pages, backend to Cloudflare Workers, and mobile builds to their respective distribution channels (Play Store/App Store) using GitHub Secrets.
