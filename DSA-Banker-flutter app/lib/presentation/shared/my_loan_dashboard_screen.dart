...
class _MyLoanDashboardScreenState extends State<MyLoanDashboardScreen> {
  ...
  final List<Map<String, dynamic>> _loanCategories = [
    ...
    {'title': 'Credit Card', 'icon': Icons.credit_card, 'color': const Color(0xFFFDCB6E)},
    {'title': 'Commercial Vehicle', 'icon': Icons.local_shipping, 'color': const Color(0xFF6C5CE7)},
    {'title': 'Car Loan', 'icon': Icons.directions_car, 'color': const Color(0xFF00CEC9)},
    {'title': 'Loan Transfer', 'icon': Icons.currency_exchange, 'color': const Color(0xFFD63031)},
  ];

  final List<Map<String, dynamic>> _insuranceCategories = [
    {'title': 'Health Insurance', 'icon': Icons.health_and_safety, 'color': const Color(0xFFE84393)},
    {'title': 'General Insurance', 'icon': Icons.shield, 'color': const Color(0xFF27AE60)},
    {'title': 'Vehicle Insurance', 'icon': Icons.directions_car, 'color': const Color(0xFF0984E3)},
  ...
  Future<void> _fetchUserName() async {
    try {
      ...
      String name = (p['name'] ?? ApiClient.currentUserName ?? 'User').toString();
      if (name.contains(' ')) name = name.split(' ')[0];
      if (mounted) setState(() => _userName = name);
    } catch (_) {
      String name = (ApiClient.currentUserName ?? 'User').toString();
      if (name.contains(' ')) name = name.split(' ')[0];
      if (mounted) setState(() => _userName = name);
    }
  }

  void _showComingSoon(String service) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$service application coming soon!'))); }
  void _openEmiCalculator() { showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (context) => const EmiCalculatorBottomSheet()); }
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        _buildHeader(), const SizedBox(height: 20), _buildUtilityBanner(), const SizedBox(height: 24),
        _buildSectionTitle('Apply For Loan'), _buildLoanGrid(), const SizedBox(height: 24),
        _buildSectionTitle('Apply For Insurance'), _buildInsuranceGrid(), const SizedBox(height: 24),
        _buildSectionTitle('Recent Applications'), _buildApplicationsList(), const SizedBox(height: 40),
      ]),
    );
  }
  ...
  Widget _buildHeader() {
    return Container(
      ...
      decoration: const BoxDecoration(color: Colors.white, borderRadius: BorderRadius.only(bottomLeft: Radius.circular(30), bottomRight: Radius.circular(30)), boxShadow: [BoxShadow(color: Colors.black12, blurRadius: 10, offset: Offset(0, 4))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
  ...
  Widget _buildApplicationsList() {
    return Padding(padding: const EdgeInsets.symmetric(horizontal: 20), child: Container(width: double.infinity, padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), border: Border.all(color: Colors.grey[200]!)), child: Column(children: [Icon(Icons.description_outlined, size: 48, color: Colors.grey[400]), const SizedBox(height: 16), Text('No recent loan applications', style: TextStyle(color: Colors.grey[600], fontSize: 16)), const SizedBox(height: 8), Text('Apply for a loan above to track its status here.', textAlign: TextAlign.center, style: TextStyle(color: Colors.grey[400], fontSize: 13))])));
  }
}
...
class _EmiCalculatorBottomSheetState extends State<EmiCalculatorBottomSheet> {
  ...
  @override
  Widget build(BuildContext context) {
    return Container(padding: const EdgeInsets.all(24), decoration: const BoxDecoration(color: Colors.white, borderRadius: BorderRadius.only(topLeft: Radius.circular(32), topRight: Radius.circular(32))), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('EMI Calculator', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), IconButton(icon: const Icon(Icons.close), onPressed: () => Navigator.pop(context))]),
      const SizedBox(height: 24),
      _buildSliderRow('Loan Amount (₹)', _principal, 10000, 5000000, (val) => setState(() => _principal = val), '₹${_principal.toInt()}'),
      ...
      Container(width: double.infinity, padding: const EdgeInsets.all(24), decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF4A3AFF), Color(0xFF6C5DD3)], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(20)), child: Column(children: [
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Principal', style: TextStyle(color: Colors.white70, fontSize: 12)), Text('₹${_principal.toInt()}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))]),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [const Text('Total Interest', style: TextStyle(color: Colors.white70, fontSize: 12)), Text('₹${(_emi * _tenureYears * 12 - _principal).toInt()}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))]),
        ]),
      ])),
      const SizedBox(height: 16),
    ]));
