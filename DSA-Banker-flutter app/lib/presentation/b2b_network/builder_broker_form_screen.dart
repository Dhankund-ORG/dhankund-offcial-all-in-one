...
class _BuilderBrokerFormScreenState extends State<BuilderBrokerFormScreen> {
  ...
  void dispose() {
    ...
    _addressController.dispose(); _currentExpController.dispose(); _totalExpController.dispose(); _aboutController.dispose();
    super.dispose();
  }
  ...
  Future<void> _submitForm() async {
    ...
    try {
      final details = {
        ...
        'company': _companyController.text, 'officeAddress': _addressController.text, 'currentExp': _currentExpController.text,
        'totalExp': _totalExpController.text, 'segment': _selectedSegment, 'profession': _selectedProfession, 'about': _aboutController.text,
        'role': widget.role,
      };
      final roleKey = (widget.role.toLowerCase() == 'customer') ? 'customer' : 'partner';
      await _api.submitRegistration(role: roleKey, details: details);
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text('${widget.role} Registration', style: const TextStyle(fontWeight: FontWeight.bold)), centerTitle: true, backgroundColor: Colors.transparent, elevation: 0),
      body: SingleChildScrollView(padding: const EdgeInsets.all(20.0), child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _buildFieldLabel('Office Address *'), _buildTextField(_addressController, 'Enter complete address', Icons.location_on, maxLines: 3),
        Row(children: [
        ...
        _buildFieldLabel('Profile Note / About'), _buildTextField(_aboutController, 'Briefly describe your profile', Icons.note, maxLines: 4),
        const SizedBox(height: 32),
        CustomButton(text: 'Submit Registration', onPressed: _submitForm, isLoading: _isLoading),
