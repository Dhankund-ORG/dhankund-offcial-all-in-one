...
class _GeneralLoanFormScreenState extends State<GeneralLoanFormScreen> {
      ...
      return Dialog(
        ...
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 16),
            InkWell(
              onTap: () => _makePhoneCall('9669084666'),
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
                decoration: BoxDecoration(color: const Color(0x0D4A3AFF), borderRadius: BorderRadius.circular(10), border: Border.all(color: const Color(0x264A3AFF))),
                child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                  const Icon(Icons.phone_in_talk, color: Color(0xFF4A3AFF), size: 18),
                  const SizedBox(width: 8),
                  Expanded(child: Text('Call us: 9669084666', textAlign: TextAlign.center, style: const TextStyle(fontSize: 13, color: Colors.black54, fontWeight: FontWeight.w600))),
                ]),
              ),
            ),
            const SizedBox(height: 24),
            SizedBox(width: double.infinity, height: 50, child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF4A3AFF), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), elevation: 0),
  ...
  Widget _buildHeader() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), boxShadow: const [BoxShadow(color: Color(0x08000000), blurRadius: 16, offset: Offset(0, 4))]),
      child: Column(children: [
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 6), decoration: BoxDecoration(color: const Color(0x144A3AFF), borderRadius: BorderRadius.circular(20)), child: Text(widget.loanTitle + ' Application', style: const TextStyle(fontSize: 12, color: Color(0xFF4A3AFF), fontWeight: FontWeight.bold))),
      ]),
    );
  }
  ...
  Widget _buildFormCard() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: const [BoxShadow(color: Color(0x0A000000), blurRadius: 16, offset: Offset(0, 4))]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _buildTextField(controller: _mobileController, hint: 'Enter 10-digit mobile number', keyboardType: TextInputType.phone, validator: (v) => (v == null || !RegExp(r'^[0-9]{10}$').hasMatch(v.trim())) ? 'Enter a valid 10-digit number' : null),
        const SizedBox(height: 20),
        _buildLabel('Mail ID (Optional)'),
        _buildTextField(controller: _emailController, hint: 'Enter your email address', keyboardType: TextInputType.emailAddress),
        const SizedBox(height: 20),
        _buildLabel('Loan Amount *'),
        _buildTextField(controller: _amountController, hint: 'Enter required loan amount', prefix: 'Rs ', keyboardType: TextInputType.number, validator: (v) { if (v == null || v.trim().isEmpty) return 'Please enter loan amount'; final n = double.tryParse(v.trim()); return (n == null || n <= 0) ? 'Enter a valid amount' : null; }),
  ...
  Widget _buildTextField({required TextEditingController controller, required String hint, String? prefix, TextInputType keyboardType = TextInputType.text, String? Function(String?)? validator}) {
    return TextFormField(controller: controller, keyboardType: keyboardType, validator: validator, style: const TextStyle(fontSize: 15, color: Colors.black87), decoration: InputDecoration(
      hintText: hint, prefixText: prefix, hintStyle: TextStyle(color: Colors.grey.shade400, fontSize: 14), filled: true, fillColor: Colors.grey.shade50, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
