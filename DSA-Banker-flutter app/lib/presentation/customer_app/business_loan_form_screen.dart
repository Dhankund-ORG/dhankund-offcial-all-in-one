...
class _BusinessLoanFormScreenState extends State<BusinessLoanFormScreen> {
  ...
  @override
  void dispose() { _nameController.dispose(); _mobileController.dispose(); _emailController.dispose(); _amountController.dispose(); _turnoverController.dispose(); super.dispose(); }
  ...
  Future<void> _submitApplication() async {
    ...
    try {
      await _api.createLoan({
        'loan_type': 'Business Loan', 'full_name': _nameController.text.trim(), 'mobile_number': _mobileController.text.trim(),
        'email': _emailController.text.trim(), 'loan_amount': _amountController.text.trim(), 'turnover': _turnoverController.text.trim(), 'status': 'Pending',
      });
    ...
    return Scaffold(
      ...
      body: SafeArea(child: SingleChildScrollView(physics: const BouncingScrollPhysics(), padding: const EdgeInsets.symmetric(horizontal: 24.0, vertical: 8.0), child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
        ...
        Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 16, offset: const Offset(0, 4))]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 20), _buildLabel('Mobile No. *'), _buildTextField(controller: _mobileController, hint: 'Enter 10-digit mobile number', keyboardType: TextInputType.phone, validator: (v) => (v == null || !RegExp(r'^[0-9]{10}$').hasMatch(v.trim())) ? 'Please enter a valid 10-digit number' : null),
          const SizedBox(height: 20), _buildLabel('Mail ID (Optional)'), _buildTextField(controller: _emailController, hint: 'Enter your email address', keyboardType: TextInputType.emailAddress, validator: (v) { if (v != null && v.trim().isNotEmpty && !RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$').hasMatch(v.trim())) return 'Please enter a valid email address'; return null; }),
          const SizedBox(height: 20), _buildLabel('Loan Amount *'), _buildTextField(controller: _amountController, hint: 'Enter required loan amount', prefix: '₹ ', keyboardType: TextInputType.number, validator: (v) { if (v == null || v.trim().isEmpty) return 'Please enter loan amount'; final n = double.tryParse(v.trim()); return (n == null || n <= 0) ? 'Please enter a valid loan amount' : null; }),
          const SizedBox(height: 20), _buildLabel('Turnover *'), _buildTextField(controller: _turnoverController, hint: 'Enter your annual turnover', prefix: '₹ ', keyboardType: TextInputType.number, validator: (v) { if (v == null || v.trim().isEmpty) return 'Please enter your annual turnover'; final n = double.tryParse(v.trim()); return (n == null || n <= 0) ? 'Please enter a valid turnover amount' : null; }),
          const SizedBox(height: 32),
