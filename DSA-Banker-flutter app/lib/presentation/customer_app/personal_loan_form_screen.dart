...
class _PersonalLoanFormScreenState extends State<PersonalLoanFormScreen> {
  ...
  @override
  void dispose() { _nameController.dispose(); _mobileController.dispose(); _emailController.dispose(); _amountController.dispose(); _salaryController.dispose(); super.dispose(); }
  ...
  Future<void> _submitApplication() async {
    ...
    try {
      await _api.createLoan({
        'loan_type': 'Personal Loan', 'full_name': _nameController.text.trim(), 'mobile_number': _mobileController.text.trim(),
        'email': _emailController.text.trim(), 'loan_amount': _amountController.text.trim(), 'salary': _salaryController.text.trim(), 'status': 'Pending',
      });
    ...
    return Scaffold(
      ...
      body: SafeArea(child: SingleChildScrollView(physics: const BouncingScrollPhysics(), padding: const EdgeInsets.symmetric(horizontal: 24.0, vertical: 8.0), child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
        ...
        Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 16, offset: const Offset(0, 4))]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 20), _buildLabel('Salary *'), _buildTextField(controller: _salaryController, hint: 'Enter your monthly salary', prefix: '₹ ', keyboardType: TextInputType.number, validator: (v) { if (v == null || v.trim().isEmpty) return 'Please enter your monthly salary'; final n = double.tryParse(v.trim()); return (n == null || n <= 0) ? 'Please enter a valid salary' : null; }),
          const SizedBox(height: 32),
