...
async function upsertUserFromRegistration(env, uid, role, userDetails) {
  ...
  delete data._source_collection;
...
app.get('/api/health', function (c) {
  return c.json({ ok: true, service: 'dhankund-crm-api' });
});

app.get('/api', function (c) {
  return c.json({ name: 'Dhankund CRM API', version: '1.0.0', endpoints: routeIndex() });
});

app.get('/api/openapi.json', function (c) {
...
app.post('/api/v1/auth/login', async function (c) {
  ...
  if (!user) {
    ...
    if (adminEmail && email === adminEmail && password === adminPassword) {
      ...
      await insertRow(c.env, 'users', {
        id: uid, email: email, password_hash: hash, role: 'admin', name: 'Admin', mobile: null,
        kyc_completed: 0, bank_details_completed: 0,
        data: JSON.stringify({ uid: uid, role: 'admin', profileCompleted: true }), created_at: now, updated_at: now
      });
  ...
  if (user.role !== 'admin' && user.role !== 'staff') {
    return c.json({ error: 'Only admin or staff accounts can access the CRM' }, 403);
  }
...
app.get('/api/v1/auth/me', auth, async function (c) {
  ...
  return c.json({ id: user.id, email: user.email, role: user.role, name: user.name });
});

app.post('/api/v1/auth/logout', auth, function (c) {
...
app.get('/api/v1/overview', auth, async function (c) {
  const env = c.env;
...
app.get('/api/v1/registrations', auth, async function (c) {
...
app.patch('/api/v1/registrations/:collection/:id', auth, async function (c) {
...
app.get('/api/v1/kyc-bank-users', auth, async function (c) {
  const rows = await all(c.env, 'SELECT * FROM users ORDER BY created_at DESC');
  return c.json(rows.map(rowToUser));
});

app.patch('/api/v1/users/:uid/kyc', auth, async function (c) {
...
app.patch('/api/v1/users/:uid/bank', auth, async function (c) {
  const uid = c.req.param('uid');
  const body = await readJson(c);
...
app.delete('/api/v1/users/:uid', auth, async function (c) {
  const uid = c.req.param('uid');
  const body = await readJson(c);
  await run(c.env, 'DELETE FROM users WHERE id = ?', [uid]);
  if (body.docId) {
    await run(c.env, 'DELETE FROM registrations WHERE id = ?', [String(body.docId)]);
  }
  return c.json({ success: true });
});

app.get('/api/v1/loans', auth, async function (c) {
...
app.post('/api/v1/loans', auth, async function (c) {
  ...
  const id = randomId();
  await insertRow(c.env, 'loan_applications', Object.assign({ id: id, submitted_at: nowIso() }, loanColumns(b)));
  return c.json({ success: true, id: id });
});

app.put('/api/v1/loans/:id', auth, async function (c) {
  const id = c.req.param('id');
  const b = await readJson(c);
...
app.patch('/api/v1/loans/:id/status', auth, async function (c) {
  const id = c.req.param('id');
  const b = await readJson(c);
  await updateRow(c.env, 'loan_applications', 'id', id, { status: (b.status || 'Pending').toString() });
  return c.json({ success: true });
});

app.get('/api/v1/referrals', auth, async function (c) {
...
app.patch('/api/v1/referrals/:id/status', auth, async function (c) {
  const id = c.req.param('id');
  const b = await readJson(c);
...
app.post('/api/v1/admin-posts', auth, async function (c) {
  const b = await readJson(c);
  const id = randomId();
  await insertRow(c.env, 'admin_posts', {
    id: id,
    uid: 'admin',
    name: 'Admin',
    title: b.title || null,
...
app.post('/api/v1/bank-policies', auth, async function (c) {
  const b = await readJson(c);
  const id = randomId();
...
app.put('/api/v1/bank-policies/:id', auth, async function (c) {
  const id = c.req.param('id');
  const b = await readJson(c);
...
app.get('/api/v1/broadcasts', auth, async function (c) {
...
app.post('/api/v1/broadcasts', auth, async function (c) {
  const b = await readJson(c);
  const id = randomId();
  ...
  await insertRow(c.env, 'broadcast_history', {
    ...
    send_whatsapp: b.send_whatsapp ? 1 : 0,
    send_email: b.send_email ? 1 : 0,
    send_push: sendPush ? 1 : 0,
    subject: b.subject || null,
    message: b.message || null,
    recipient_count: (b.recipient_count != null) ? b.recipient_count : 0,
    timestamp: nowIso()
  });
  ...
  if (sendPush) {
    ...
    for (const t of tokens) {
      try {
        await sendFcm(c.env, { token: t.token, title: b.subject || 'Dhankund', body: b.message || '', data: {} });
        pushSent += 1;
      } catch (e) {
...
app.post('/api/v1/push', auth, async function (c) {
  const b = await readJson(c);
  if (!b.token) return c.json({ error: 'token is required' }, 400);
